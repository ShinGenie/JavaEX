...
public class MClient_CHADAN extends JFrame implements ActionListener, Runnable{
	...
	private JLabel jlab_say3=new JLabel("안녕",JLabel.CENTER);
	...
	JLabel jlab_login_img;
	JLabel jlab_other_img;
	...
	public void layoutSet() { // 화면 디자인 넣어주세요
		...
		ImageIcon img1=new ImageIcon();  //이미지 가져오기
		jlab_login_img=new JLabel(img1);
		...
		ImageIcon img2=new ImageIcon();  //이미지 가져오기
		jlab_other_img=new JLabel(img2);
//		jlab_other_img.setIcon(new ImageIcon("images/imo1.jpg"));
		
		jlab_name3.setForeground(new Color(255, 94, 128));
                 ...
                 jlab_hobby2.setForeground(new Color(255,94,128));
            	  ...
            	  insertString(doc.getLength(),time+"\r\n",doc.getStyle("small"));
						...
						insertString(doc.getLength(),time+"\r\n",doc.getStyle("small"));
							...
							//String [] ar=(String[])msg.substring(msg.indexOf("-")+1);
...
//		            	  switch(k) {
//		            	  case "true":
//		            		  jlab_other_img.setIcon(new ImageIcon("images/2cat.png"));
//		            		  break;
//		            	  default : 
//		            		  jlab_other_img.setIcon(new ImageIcon("images/1cat.png"));
//		            	  }
//		            	  
//		            	  if(k.equals("true")) jlab_other_img.setIcon(new ImageIcon("images/2cat.png"));
//		            	  else jlab_other_img.setIcon(new ImageIcon("images/1cat.png"));
		            	  //if(arr[4].equals("true")||arr[4]=="true")
		            	  //jlab_other_img.setIcon(new ImageIcon("images/imo1.jpg"));
...
//		else if(list.){}
//		else if(list.getSelectedItem().equals(myname)) {
//			
//			System.out.println("테스트중입니다."+list.getSelectedItem()	);
//		
//		}
	...
	public void saveUserData() {
	 ...
	 public void fileLoad() { //대화 내용 불러오기 함수
	      ...
	      fd.show();
	      String dir=fd.getDirectory();      //주소 구하기
	      String file=fd.getFile();                  // 파일이름
	      if(dir==null||file==null) return;
	      try{
	         FileReader fr=new FileReader(dir+file);         //파일안에 정보 읽어오기 
	         BufferedReader br=new BufferedReader(fr);      //불러온걸 버퍼리더로 읽어드리는것같음
	         while(true){
	            String data="";
	            data=br.readLine();                                          // 시간이용하여  if써서 시간에 맞게 집어 넣어도 괜찮을 듯
	            if(data==null)break;
	            insertString(data+"\n");
	...
	public void connect() {	//서버 접속
		...
		try {
			...
			if(cb_m.isSelected()) {
				gender=false; 
				jlab_login_img.setIcon(new ImageIcon("images/1cat.png"));
			}
			else if(cb_f.isSelected()) {
				gender=true;
				jlab_login_img.setIcon(new ImageIcon("images/2cat.png"));
			}
			out.write((jtf_name.getText()+"/"+jtf_age.getText()+"/"+jtf_hobby.getText()+"/"+jlab_say.getText()+"/"+gender.toString() + "\n").getBytes());
	...
	public static void main(String[] args) {
		MClient_CHADAN mainCl = new MClient_CHADAN();
